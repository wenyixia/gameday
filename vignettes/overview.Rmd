...
After you install and load the `gameday` package, in your `R Console`, you could type `?gday` to read the documentation of this function. There are two arguments in the `gday()` function. That is `team` and `date`.
...
If your input date is an invalid date, it will give you an error like this.
...
The [Class Activity 1] and [Class Activity 2] lead us to a package that is half-made. Based on it, I recorded my workflow below.

#### Helper function `Internet_connection()`
Following the instructions of the [Homework in Package], I first tried to add the function `Internet_connection()`. I copied the function that is already provided in the homework.
...
I did not use `assertthat` as suggested in the [Homework](), since `if()` and `stop()` have more informative return. Then I also add tests about `gday()`. 
